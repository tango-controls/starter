...
//=============================================================
    ...
    void ProcessData::read_process_list_from_sys() {
        ...
        do {
            ...
            if( hProcess != nullptr )
            {
                ...
                DWORD		dwSize=0;
                if (lpfnNtQueryInformationProcess != nullptr)
        ...
        do {
            ...
            if( hProcess != nullptr ) {
                ...
                if (rtlUserProcParamsAddress!=nullptr)
                {
                    ...
                    if (ReadProcessMemory(hProcess, commandLine.Buffer,
                        commandLineContents, commandLine.Length, nullptr)) {
    ...
    UNICODE_STRING ProcessData::getUnicodeCommandLine(HANDLE hProcess, PVOID paramAddress) {
        ...
        ReadProcessMemory(hProcess,
            ...
            &commandLine, sizeof(commandLine), nullptr);
#else
        ReadProcessMemory(hProcess,
...
//=============================================================
    bool ProcessData::manageProcFiles(Process *process) {
        ...
        if (ifs) {
            ...
            //	Replace nullptr with SPACE char
...
//=============================================================
    void ProcessData::check_cpp_process(Process *process) {
        //	Remove path
#ifndef _TG_WINDOWS_
        if (process->line_args.empty())
            process->name = "";
        else
            process->name = name_from_path(process->line_args[0]);
#endif

        for (unsigned long i = 1; i < process->line_args.size(); i++)
...
//=============================================================
    bool ProcessData::check_java_process(Process *process) {
        ...
        for (unsigned long i = process->line_args.size() - 1; !found && i > 0; i--) {
            if (!process->line_args[i].empty() &&
                 process->line_args[i].c_str()[0] != '-') {
...
//		return false;
        ...
        for (unsigned long i = args_idx; i < process->line_args.size(); i++)
            process->proc_args.push_back(process->line_args[i]);
        return true;
    }
...
//=============================================================
//=============================================================
    string ProcessData::name_from_path(const string& fullName) {
        string::size_type start = 0;
        string::size_type end;
        while ((end = fullName.find('/', start)) != string::npos)
            start = end + 1;
        //	Get last one
        ...
        for (auto process : proc_list) {
            ...
            while ((end = process->line.find(' ', start)) != string::npos) {
                ...
                if (!s.empty() && s != " " && s != "\t") {
                    ...
                    if (!inCotes && s.find('\"') != string::npos) //	starting
                    {
                        process->line_args.push_back(s);
                        inCotes = true;
                    } else if (inCotes ) {// inside
            ...
            if (!check_java_process(process))
                if (!check_python_process(process))
...
//=============================================================
//=============================================================
    int ProcessData::get_server_pid(const string& argIn) {
        omni_mutex_lock sync(*this);
 ...
 */
//=============================================================
    bool ProcessData::is_server_running(const string& argIn) {
        omni_mutex_lock sync(*this);
        for (Process *process : proc_list) {
 ...
 */
//=============================================================
    bool ProcessData::is_process_running(const string& argIn) {
        omni_mutex_lock sync(*this);
        for (Process *process : proc_list) {
 ...
 */
//=============================================================
    int ProcessData::getNbServerInstances(const string& argIn) {
        omni_mutex_lock sync(*this);
        int nb = 0;
        for (Process *process : proc_list) {
            if (!process->servname.empty() && process->servname == argIn)
...
//=============================================================
    vector<Process> ProcessData::get_process_list() {
        ...
        //	copy list
        vector<Process> vReturn;
        for (Process *p_src : proc_list) {
            Process process;
            Process *p_target = &process;
            *p_target = *p_src;
            vReturn.push_back(process);
        }
...
//=============================================================
//=============================================================
    int CheckProcessUtil::get_server_pid(const string& argIn) {
 ...
 */
//=============================================================
    bool CheckProcessUtil::is_server_running(const string& argIn) {
 ...
 */
//=============================================================
    bool CheckProcessUtil::is_process_running(const string& argIn) {
 ...
 */
//=============================================================
    int CheckProcessUtil::getNbServerInstances(const string& argIn) {
