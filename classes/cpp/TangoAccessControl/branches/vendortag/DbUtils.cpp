	...
	TangoAccessControl::get_access_for_user_address(string &user, string &ip_add)
{
	...
	DEBUG_STREAM << "ac_get_access(): sql_query " << sql_query_stream.str() << endl;
...
string TangoAccessControl::get_access_for_user_device(string &user, string &device)
{
	...
	TangoSys_MemStream	sql_query_stream;
	sql_query_stream << 
		"SELECT DISTINCT device,rights FROM access_device WHERE "  <<
			//	User definition
				"user=\"" << user << "\"  AND  "    <<
			//	Device definition
				"(device=\"*/*/*\" OR "                                 <<
	...
	if (n_rows > 0)
	{
		for (int i=0; i<n_rows; i++)
		{
			...
			if (row != NULL)
			{
				...
				acs.device = row[0];
				acs.rights = row[1];
				DEBUG_STREAM << "		object[" << i << "} : " << acs.device
					<< "|" << acs.rights << endl;
				retval = acs.rights;
				as_read.push_back(acs);
			}
...
void TangoAccessControl::register_service(string servicename, string instname, string devname)
{
	...
	Tango::DbData	data;
	data.push_back(Tango::DbDatum("Services"));
	Tango::Util *tg = Tango::Util::instance();
	tg->get_database()->get_property("ControlSystem", data);
	...
	data[0] << services;
	tg->get_database()->put_property(CtrlSystem, data);
}
//============================================================
//============================================================
