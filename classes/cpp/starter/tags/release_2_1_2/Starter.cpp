...
const bool Starter::is_running(ServerControled *server)
{
	...
	try
	{
		if (server->dev == NULL)
		{
			...
			server->dev->set_timeout_millis(PING_TIMEOUT);
		}
...
#ifdef DisplayTime
	GetTime(after);
	if (Elapsed(before, after) > 100)
	cout << "ping " << server->admin_name << "  " << Elapsed(before, after) << " ms" << endl;
#endif
	return retval;
}
...
void Starter::get_device_property()
{
	...
	//	Extract WaitForDriverStartup value
	...
	//	Extract StartServersAtStartup value
	...
	//	Extract InterStartupLevelWait value
...
//	Tango::DeviceProxy	*dev = new Tango::DeviceProxy(server->admin_name);
	...
	try
	{
		//	increase timeout (minimum for ping)
		server->dev->set_timeout_millis(2000);
		server->dev->command_inout("Kill");
		WARN_STREAM << argin << " stopped !" << endl;
		cout << argin << " stopped !" << endl;
		//	restore timeout for ping
		server->dev->set_timeout_millis(PING_TIMEOUT);
	}
	catch(Tango::DevFailed &e)
	{
		for (int i=0 ; i<e.errors.length() ; i++)
		{
			...
			if (reason=="API_CommandNotFound")
			{
				...
				server->dev->command_inout("DevKill");
				//	restore timeout for ping
				server->dev->set_timeout_millis(PING_TIMEOUT);
				return;
			}
