...
{
	//	wait a bit for dserver exported.
	ms_sleep(1000);
	long	upd_period = 1000;
	add_obj_polling("attribute", "State",          upd_period);
	add_obj_polling("attribute", "HostState",      upd_period);
	add_obj_polling("attribute", "RunningServers", upd_period);
	add_obj_polling("attribute", "StoppedServers", upd_period);
	return NULL;
}
 ...
 */
//+------------------------------------------------------------------
void PollingState::add_obj_polling(string type, string objname, int upd_period)
{
	Tango::Util *tg = Tango::Util::instance();
