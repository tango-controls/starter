...
void StarterUtil::build_server_ctrl_object(vector<ServerControled> *servers)
{
	...
	if (one_call)
	{
		...
		{
			Tango::Except::print_exception(e);
			//	Check if it is due to a prvious DatabaseDs version
	...
	if (one_call)
	{
		...
		{
			cout << "----------------------------------------" << endl;
			cout << (result.size()/3) << " servers found" << endl;
			servers->clear();
