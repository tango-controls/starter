...
Tango::DevState StarterUtil::is_notifyd_alive()
{
	try
	{
		...
		bool  	factory_exported = (lsa->lvalue[0]==1);
		if (factory_exported==false)
		{
			cout2 << notifyd_name << " NOT exported !" << endl;
...
*/
		...
		{
			cout << notifyd_name << " is not an EventChannelFactory " << endl;
 			return Tango::DevState(Tango::FAULT);
		}
		else
	...
	{
		cout2 << "Failed to narrow the EventChannelFactory on " << notifyd_name << endl;
		cout2 << " make sure the notifd process is running on this host" << endl;
 		return Tango::DevState(Tango::FAULT);
	}
	//	Never used.
	return Tango::DevState(Tango::ON);
...
void *PollingState::run_undetached(void *ptr)
{
	...
	while(true)
	{
		...
		//	If a command has not been received for a l;ong time 
