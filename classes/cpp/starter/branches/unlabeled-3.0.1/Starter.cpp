...
void Starter::init_device()
{
	...
	if (starting==true)
	{
		...
		attr_ControlledRunningServers_read =
						new Tango::DevString[MAX_SERVERS_BY_HOST];
		attr_ControlledStoppedServers_read =
						new Tango::DevString[MAX_SERVERS_BY_HOST];
		attr_AllRunningServers_read =
						new Tango::DevString[MAX_SERVERS_BY_HOST];
		attr_AllStoppedServers_read =
						new Tango::DevString[MAX_SERVERS_BY_HOST];
...
void Starter::get_device_property()
{
	...
	data.push_back(Tango::DbDatum("DisableNotifyDaemon"));
	...
	if (data[2].is_empty()==false)	data[2]  >>  disableNotifyDaemon;
	...
	INFO_STREAM << "DisableNotifyDaemon  = " << ((disableNotifyDaemon==false)? "False": "True") << endl;
...
void Starter::read_attr(Tango::Attribute &attr)
{
	...
	if (attr_name == "HostState")
	{
		...
		cout << "HostState = " << attr_HostState_read[0] << endl;
		attr.set_value(attr_HostState_read);
	}
	else
	if (attr_name == "ControlledRunningServers")
	{
		int		nb = 0, i;
		...
		if (servers.size()>0)
		{
			...
			for (i=0 ; i<servers.size() ; i++)
				if (servers[i].controled && servers[i].running)
				{
					INFO_STREAM << "RUNNING: " << servers[i].name << endl;
					attr_ControlledRunningServers_read[nb++] =
							CORBA::string_dup(servers[i].name.c_str());
				}
		}
		attr.set_value(attr_ControlledRunningServers_read, nb);
	}
	else
	if (attr_name == "ControlledStoppedServers")
	{
		int		nb = 0, i;
		//	Check if servers object initilized
		//---------------------------------------
		if (servers.size()>0)
		{
			...
			for (i=0 ; i<servers.size() ; i++)
				if (servers[i].controled && !servers[i].running)
				{
					...
					attr_ControlledStoppedServers_read[nb++] =
							CORBA::string_dup(servers[i].name.c_str());
				}
		}
		attr.set_value(attr_ControlledStoppedServers_read, nb);
	}
	else
	if (attr_name == "AllRunningServers")
	{
		int		nb = 0, i;
		//	Check if servers object initilized
		//---------------------------------------
		if (servers.size()>0)
		{
			...
			for (i=0 ; i<servers.size() ; i++)
				...
				{
					INFO_STREAM << "RUNNING: " << servers[i].name << endl;
					attr_AllRunningServers_read[nb++] =
							CORBA::string_dup(servers[i].name.c_str());
				}
		}
		attr.set_value(attr_AllRunningServers_read, nb);
	}
	else
	if (attr_name == "AllStoppedServers")
	{
		int		nb = 0, i;
		//	Check if servers object initilized
		//---------------------------------------
		if (servers.size()>0)
		{
			...
			//-----------------------------------------------------------
			for (i=0 ; i<servers.size() ; i++)
				if (servers[i].controled)
				{
					INFO_STREAM << "STOPPED: " << servers[i].name << endl;
					attr_AllStoppedServers_read[nb++] =
							CORBA::string_dup(servers[i].name.c_str());
				}
		}
		attr.set_value(attr_AllStoppedServers_read, nb);
	}
}
...
void Starter::manage_PollingState_startup()
{
	...
	if (poller == NULL)
	{
		...
		/**
		//	Fist time stop polling
		try
		{
			CORBA::Any	any;
			adm_dev->command_inout("StopPolling", any);
		}
		catch (Tango::DevFailed &e) {
			Tango::Except::print_exception(e);
		}
		*/
		...
		//	Add State command as polled object
		...
		lsa->svalue[1] = CORBA::string_dup("command");
