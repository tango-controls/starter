...
// first delete the tuple (device,name,count) from the property table
				...
				sql_query_stream << "DELETE FROM property WHERE object=\"" << tmp_object \
												 << "\" AND name=\"" << tmp_name \
												 << "\" AND count=\"" << tmp_count_str << "\"";
				sql_query.erase();
				sql_query = sql_query_stream.str();
	      DEBUG_STREAM << "DataBase::db_put_property(): sql_query " << sql_query << endl;
	      if (mysql_real_query(&mysql, (const char *)sql_query.c_str(), \
														 strlen((const char *)sql_query.c_str())) != 0)
	      {
...
Tango::DevVarStringArray *DataBase::db_get_host_server_list(Tango::DevString wildcard)
{
	...
	else
	{
		...
		sprintf(sql_query,"SELECT DISTINCT server FROM device WHERE host LIKE \"%s\" or host LIKE \"%s.%%\" ORDER BY server",
...
//	        tmp_device.c_str(),tmp_device.c_str());
	...
	if (mysql_real_query(&mysql, sql_query, strlen(sql_query)) != 0)
	{
	   WARN_STREAM << "DataBase::ImportDeviceList(): failed to query TANGO database ";
	   WARN_STREAM << "for import info for device " << tmp_device << " by device name (error=" << mysql_error(&mysql) << ")" << endl;
	   sprintf(error_mess,"failed to query TANGO database for import info of device for %s by name (error=%s)",tmp_device.c_str(),mysql_error(&mysql));
	   Tango::Except::throw_exception((const char *)DB_SQLError,
	   				  (const char *)error_mess,
...
//
		...
		mysql_free_result(result);
		sprintf(sql_query,"SELECT exported,ior,version,pid,server,host FROM device WHERE alias = '%s';",
		        tmp_device.c_str());
		DEBUG_STREAM << "DataBase::ImportDevice(): sql_query " << sql_query << endl;
		...
		{
		   WARN_STREAM << "DataBase::ImportDeviceList(): failed to query TANGO database ";
		   WARN_STREAM << "for import info for device " << tmp_device << " by device alias (error=" << mysql_error(&mysql) << ")" << endl;
		   sprintf(error_mess,"failed to query TANGO database for import info of device for %s by alias (error=%s)",tmp_device.c_str(),mysql_error(&mysql));
		   Tango::Except::throw_exception((const char *)DB_SQLError,
		   				  (const char *)error_mess,
						  (const char *)"DataBase::ImportDevice()");
		}
...
// un-export event from database by seting ior="not exported"
	...
	if (mysql_real_query(&mysql, sql_query, strlen(sql_query)) != 0)
	{
   		...
   		WARN_STREAM << "for " << tmp_event << " (error=" << mysql_error(&mysql) << ")" << endl;
   		sprintf(error_mess,"failed to un-export event %s from TANGO database  (error=%s)",
					tmp_event.c_str(), mysql_error(&mysql));
   		Tango::Except::throw_exception((const char *)DB_SQLError,
					       (const char *)error_mess,
...
Tango::DevString DataBase::db_get_device_host(Tango::DevString argin)
{
	...
	if (mysql_real_query(&mysql, sql_query, strlen(sql_query)) != 0)
	{
	   WARN_STREAM << "DataBase::db_get_device_host(): failed to query TANGO database";
	...
	if ((result = mysql_store_result(&mysql)) == NULL)
	{
	   WARN_STREAM << "DataBase::db_get_device_host(): mysql_store_result() failed  (error=" << mysql_error(&mysql) << ")" << endl;
	   sprintf(error_mess,"mysql_store_result() failed (error=%s)",mysql_error(&mysql));
	   Tango::Except::throw_exception((const char *)DB_SQLError,
	   				  (const char *)error_mess,
					  (const char *)"DataBase::db_get_device_host()");
...
Tango::DevVarLongStringArray *DataBase::db_get_device_info(Tango::DevString argin)
{
	...
	sprintf(sql_query,"SELECT exported,ior,version,pid,server,host,started,stopped FROM device WHERE name = '%s' or alias = '%s';",
	        tmp_device.c_str(),tmp_device.c_str());
	DEBUG_STREAM << "DataBase::ImportDevice(): sql_query " << sql_query << endl;
	...
	if (n_rows > 0)
	{
	   ...
	   if ((row = mysql_fetch_row(result)) != NULL)
	   {
		...
		for (int x=0 ; x<2 ; x++)
		{
			...
			{
				sprintf(sql_query2,
					"SELECT DATE_FORMAT(\'%s\',\'%s\')", row[x+6], format);
				if (mysql_real_query(&mysql, sql_query2, strlen(sql_query)) == 0)
				{
